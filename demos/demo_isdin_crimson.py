"""怡思丁双抗防晒竞品分析 · crimson look（经典深红咨询风）—— 源自完整版报告(2026-07-03)。

对标 CyberPPT 咨询纪律:标题写成 SCR 结论句;分析主线用 exhibit 复合证据版面
(主图表 + 侧栏结构条 + 编号洞察框 + 底部「对企业的意义」条 + 来源);
其余内容页带 SO WHAT 结论条。数据口径:千瓜 2025.01-10 小红书商业投放。
用法: python demo_isdin_crimson.py [out.pptx] [截图目录]
"""
from __future__ import annotations
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent   # 仓库根:可导入 ppt_engine,产物统一进 out/
sys.path.insert(0, str(ROOT))
from ppt_engine.build import Engine
from ppt_engine.selfcheck import check_layout
from ppt_engine.ir import (
    Deck, DeckMeta, Slide,
    HeroData, HeroFact, TocData, TocItem, SectionData, BulletsData, Bullet,
    IconGridData, IconCard, TwoColData, Column, KpiData, Stat,
    Series, TableData, QuoteData, ClosingData,
    ExhibitData, SideModule, SideBar, InsightBox, Implication,
)

SRC = "资料来源:千瓜数据 · 小红书商业投放 2025.01–10"

DECK = Deck(
    meta=DeckMeta(title="怡思丁双抗防晒竞品分析"),
    theme="crimson",
    slides=[
        # 1 封面 hero —— 防晒主题 t2i(内容 prompt,风格由 look 强制)
        Slide(data=HeroData(
            eyebrow="ISDIN COMPETITIVE · 2026", title="双抗防晒**攻防战**",
            subtitle="怡思丁 × 蜜丝婷 × 安热沙 × 理肤泉 · 小红书五维竞品分析",
            footer="KIWOO INSIGHT · 2026-07-03 · 内部资料", art="sun",
            prompt="A tall sunscreen tube standing upright on a beach, a huge "
                   "blazing sun disc high in the sky with radiating rays, palm "
                   "leaf and beach umbrella silhouettes, calm sea horizon band "
                   "in the foreground",
            facts=[HeroFact(value="138篇", label="本品商业笔记"),
                   HeroFact(value="7.59万", label="本品互动总量"),
                   HeroFact(value="0", label="头部达人合作")])),
        # 2 目录
        Slide(data=TocData(
            eyebrow="CONTENTS", title="目录",
            items=[TocItem(title="竞品格局", desc="市场底色·五品牌对照·体量差距", pages="P03–P06"),
                   TocItem(title="打法拆解", desc="内容需求线·节奏·需求线明细", pages="P07–P09"),
                   TocItem(title="卖点攻防", desc="双抗差异化 vs 竞品概念占位", pages="P10–P11"),
                   TocItem(title="攻防建议", desc="三主攻 · 四避让", pages="P12"),
                   TocItem(title="局限与缺口", desc="五个口径缺口", pages="P13"),
                   TocItem(title="结论", desc="心智主线与收尾", pages="P14–P15")])),
        # 3 章节 01
        Slide(data=SectionData(number="01", title="竞品格局",
                               subtitle="LANDSCAPE · XHS 2025.01–10")),
        # 4 市场底色 KPI —— SCR:结论进标题,数字当证据
        Slide(data=KpiData(
            eyebrow="CORE MARKET · 2025", title="线上与社媒**主导**品类声量",
            stats=[
                Stat(value="244亿+", label="品类市场规模", delta="+10-15%", delta_dir="up"),
                Stat(value="78%", label="线上渠道占比", delta="电商主导", delta_dir="flat"),
                Stat(value="52.4%", label="抖音线上市占", delta="超越天猫", delta_dir="up"),
                Stat(value="68.6%", label="社媒信息获取", delta="KOL 56.6%", delta_dir="up"),
            ],
            so_what="244 亿品类、78% 线上、68.6% 走社媒:小红书声量质量直接决定种草效率")),
        # 5 五品牌对照表
        Slide(data=TableData(
            eyebrow="HEAD TO HEAD", title="投放对照:怡思丁**全面垫底**",
            subtitle="XHS COMMERCIAL NOTES · QIANGUA 2025.01–10",
            headers=["品牌", "笔记数", "投放金额", "互动总量", "爆文率"],
            rows=[
                ["理肤泉", "632篇", "553万", "105.7万", "52.06%"],
                ["蜜丝婷·摇摇乐", "997篇", "374万", "151.8万", "45.34%"],
                ["蜜丝婷·小黄帽", "581篇", "236万", "65.8万", "42.69%"],
                ["安热沙", "371篇", "449万", "32.4万", "34.23%"],
                ["怡思丁", "138篇", "67.6万", "7.6万", "32.61%"],
            ],
            so_what="138 篇 vs 竞品 371–997 篇:差距是结构性投入不足,不是内容质量问题")),
        # 6 复合证据版面 A —— 体量差距 × 达人结构(原图表页+对比页合并)
        Slide(data=ExhibitData(
            eyebrow="核心结论 · ENGAGEMENT",
            title="互动体量仅为竞品 **1/5–1/14**:差距源于结构性投入,「头部缺位」锁死单篇上限",
            chart_title="五品牌互动总量(万) · 2025.01–10",
            chart_type="column",
            categories=["摇摇乐", "理肤泉", "小黄帽", "安热沙", "怡思丁"],
            series=[Series(name="互动总量(万)", values=[151.8, 105.7, 65.8, 32.4, 7.6])],
            chart_note="怡思丁 7.6 万 ≈ 头部竞品的 1/14",
            side=[
                SideModule(title="爆文率对比(%)", bars=[
                    SideBar(label="理肤泉", value=52.06, note="52.1"),
                    SideBar(label="摇摇乐", value=45.34, note="45.3"),
                    SideBar(label="安热沙", value=34.23, note="34.2"),
                    SideBar(label="怡思丁", value=32.61, note="32.6", em=True)]),
                SideModule(title="投放金额(万元)", bars=[
                    SideBar(label="理肤泉", value=553, note="553"),
                    SideBar(label="安热沙", value=449, note="449"),
                    SideBar(label="摇摇乐", value=374, note="374"),
                    SideBar(label="怡思丁", value=67.6, note="68", em=True)]),
            ],
            insights=[
                InsightBox(title="竞品三种分工,天花板都被拉开", points=[
                    "理肤泉头部挑大梁:贡献 30% 互动,单篇爆文 8.77 万",
                    "蜜丝婷 365 位初级铺量贡献 53%;安热沙腰部贡献 66%"]),
                InsightBox(title="怡思丁:头部缺位锁死上限", points=[
                    "头部达人合作数 = 0,爆文率 32.61% 垫底",
                    "腰部 41 人贡献 51%,初级 86 人贡献 48%,单篇上限被锁死"]),
            ],
            implications=[
                Implication(icon="users", title="引入头部", desc="1-2 位头部达人,单篇破圈 5 万+"),
                Implication(icon="zap", title="深耕实测", desc="40°C 暴晒实测做成品牌记忆点"),
                Implication(icon="target", title="抢占场景", desc="军训/大促/泛生活场景补位"),
                Implication(icon="trending-up", title="视频优先", desc="40% 数量贡献 54% 互动"),
            ],
            source=SRC)),
        # 7 章节 02
        Slide(data=SectionData(number="02", title="打法拆解",
                               subtitle="PLAYBOOK DECODED · CONTENT × RHYTHM")),
        # 8 复合证据版面 B —— 内容需求线 × 全年节奏(原时间线页并入)
        Slide(data=ExhibitData(
            eyebrow="核心结论 · CONTENT × RHYTHM",
            title="内容分四条需求线:**极端实测**是本品唯一记忆点,Q3 声量失速 -69% 是最大缺口",
            chart_title="需求线代表作单篇互动(万)",
            chart_type="column",
            categories=["极端实测", "旅游攻略", "泛生活破圈", "军训好物"],
            series=[Series(name="单篇互动(万)", values=[0.47, 2.46, 8.77, 4.8])],
            chart_note="泛生活 8.77 万来自理肤泉头部达人",
            side=[
                SideModule(title="场景/概念占位笔记数(篇)", bars=[
                    SideBar(label="户外·蜜丝婷", value=253, note="253"),
                    SideBar(label="油皮·蜜丝婷", value=175, note="175"),
                    SideBar(label="麦色滤·理肤泉", value=171, note="171"),
                    SideBar(label="专研·怡思丁", value=52, note="52", em=True)]),
                SideModule(title="本品话题互动:Q2 → Q3(万)", bars=[
                    SideBar(label="Q2 高位", value=3.63, note="3.63"),
                    SideBar(label="Q3 失速", value=1.12, note="-69%", em=True)]),
            ],
            insights=[
                InsightBox(title="两个效率杠杆已经验证", points=[
                    "视频笔记以 40% 数量贡献 54% 互动,爆款率仍有空间",
                    "40°C 暴晒实测单篇 4689 互动,已验证高效但体量小"]),
                InsightBox(title="节奏错位:前置窗口被强占", points=[
                    "618+法网档被摇摇乐/理肤泉正面强压,摇摇乐峰值 8 万",
                    "军训开学档单篇 4.8 万互动,本品几乎未布局"]),
            ],
            implications=[
                Implication(icon="zap", title="深耕实测", desc="系列化,做成品牌记忆点"),
                Implication(icon="users", title="破圈靠头部", desc="泛生活调性需头部拉起"),
                Implication(icon="target", title="抢军训档", desc="8-9 月增量场景重点补位"),
                Implication(icon="shield", title="避让油皮", desc="高同质化赛道不正面进攻"),
            ],
            source=SRC)),
        # 9 需求线明细证据格
        Slide(data=IconGridData(
            eyebrow="EVIDENCE DETAIL", title="四条需求线,**两个杠杆**",
            subtitle="FOUR DEMAND LINES × TWO LEVERS · XHS 2025",
            cards=[
                IconCard(icon="zap", title="极端实测", subtitle="Extreme Proof",
                         lines=["40°C 暴晒实测互动 4689", "已验证高效但体量小"], punch="记忆点,深耕它"),
                IconCard(icon="target", title="旅游攻略", subtitle="Travel Scene",
                         lines=["安热沙深绑日本旅游", "猫猫岛攻略 2.46 万互动"], punch="场景要抢占"),
                IconCard(icon="lightbulb", title="泛生活破圈", subtitle="Lifestyle",
                         lines=["理肤泉人文内容 8.77 万", "调性靠头部才能拉起"], punch="破圈靠头部"),
                IconCard(icon="shield", title="护肤功效", subtitle="Skincare",
                         lines=["油皮 175 篇被蜜丝婷占", "同质化竞争激烈"], punch="避让此赛道"),
                IconCard(icon="trending-up", title="视频优先", subtitle="Video First",
                         lines=["视频 40% 数量贡 54% 互动", "爆款率有提升空间"], punch="效率杠杆"),
                IconCard(icon="users", title="军训增量", subtitle="Back To School",
                         lines=["军训好物单篇 4.8 万互动", "本品 Q3 声量 -69%"], punch="Q3 关键补位"),
            ],
            so_what="深耕极端实测记忆点;用视频杠杆与军训场景补 Q3 声量缺口")),
        # 10 卖点 hero —— 抗热老差异化
        Slide(data=HeroData(
            eyebrow="SECTION 03 · USP", title="**双抗**,独一无二",
            subtitle="抗光老 + 抗热老:有事实差异化,缺传播力度",
            footer="ANTI PHOTOAGING × ANTI HEAT AGING", art="sun",
            prompt="Close-up still life of a sunscreen bottle beside a large "
                   "vintage thermometer showing extreme heat, bold sun rays and "
                   "rising heat waves behind them",
            facts=[HeroFact(value="52篇", label="本品「专研」声量"),
                   HeroFact(value="171篇", label="理肤泉麦色滤"),
                   HeroFact(value="175篇", label="蜜丝婷油皮词")])),
        # 11 概念占位表
        Slide(data=TableData(
            eyebrow="MINDSHARE MAP", title="科技概念**占位战**已开打",
            subtitle="TECH CONCEPT × SCENE · XHS 2025.01–10",
            headers=["品牌", "科技概念", "笔记数", "核心场景"],
            rows=[
                ["理肤泉", "麦色滤 + 大哥大昵称", "171篇", "泛生活 · 军训"],
                ["蜜丝婷", "油皮 · 成膜", "175篇", "户外 253 篇"],
                ["安热沙", "#防晒大膜王", "话题绑定", "日本旅游"],
                ["怡思丁", "双抗 · 热保护科技", "专研仅52篇", "40°C 实测 · 海岛"],
            ],
            so_what="「专研」仅 52 篇:有独家事实、缺传播力度,概念资产尚未进入心智")),
        # 12 攻防清单
        Slide(data=TwoColData(
            eyebrow="PLAYBOOK 2026", title="三**主攻**,四**避让**",
            left=Column(heading="主攻 OFFENSE", points=[
                "引入 1-2 位头部达人,目标单篇破圈 5 万+",
                "深耕 40°C 暴晒实测,补军训/大促/泛生活场景",
                "主攻「抗热老」:科技概念从 52 篇冲 150+ 篇",
                "视频优先:不加投放量也能放大互动"]),
            right=Column(heading="避让 DEFENSE", points=[
                "通勤/油皮/养肤:被蜜丝婷强占,高同质化",
                "618 前置窗口:理肤泉/摇摇乐正面强压",
                "初级铺量模式:365 人体量,预算不支撑",
                "网球 IP 未确认授权,品牌资产线慎动"]),
            so_what="预算有限时先买头部与抗热老心智,正面战场全部避让")),
        # 13 局限与证据缺口
        Slide(data=BulletsData(
            eyebrow="CAVEATS", title="证据有界:五个口径缺口",
            bullets=[
                Bullet(text="搜索占位数据缺失:无法判断搜索入口的拦截关系", emphasis=True),
                Bullet(text="人群画像缺失:候选人群为内容样本反推,缺直接验证", emphasis=True),
                Bullet(text="时间窗止于 10 月:双 11 节点表现未纳入"),
                Bullet(text="理肤泉/安热沙达人层级与分形式互动数据不完整"),
                Bullet(text="仅覆盖小红书;抖音已成线上第一渠道,跨平台需另行分析"),
            ],
            so_what="补齐搜索占位与人群验证数据后,再定投放盘量级")),
        # 14 金句
        Slide(data=QuoteData(
            quote="双抗写在配方里,也要写进**心智**里。", attribution="—— 2026 攻防主线")),
        # 15 收尾
        Slide(data=ClosingData(
            title="把**差异化**讲出来。", subtitle="Own the anti-heat-aging story",
            contact="怡思丁 2026 小红书种草策略 · 内部资料")),
    ],
)


if __name__ == "__main__":
    dest = ROOT / "out" / "isdin_crimson"
    out = sys.argv[1] if len(sys.argv) > 1 else str(dest / "isdin_crimson.pptx")
    shots = sys.argv[2] if len(sys.argv) > 2 else str(dest / "preview")
    Path(out).parent.mkdir(parents=True, exist_ok=True)
    Path(shots).mkdir(parents=True, exist_ok=True)
    prims = Engine().build(DECK, out, screenshot_dir=shots)
    print(f"已生成 {out} — {len(prims)} 页, {sum(len(p) for p in prims)} 原语")
    print("结构自检:", check_layout(prims) or "零问题 ✓")
