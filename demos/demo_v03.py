...
无 key → 自动降级确定性 seed。截图写到 out/v03_freedom/preview/，pptx 写到 out/v03_freedom/。"""
import sys
...
def main():
    ...
    dest = ROOT / "out" / "v03_freedom"
    shots = dest / "preview"; shots.mkdir(parents=True, exist_ok=True)
    out = str(dest / (topic[:16].replace(" ", "_") + ".pptx"))
