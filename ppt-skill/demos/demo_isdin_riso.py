"""怡思丁双抗防晒竞品分析 · riso look —— 源自完整版报告(2026-07-03)。

数据口径:千瓜 2025.01-10 小红书商业投放;详见报告参考文献。
用法: python demo_isdin_riso.py [out.pptx] [截图目录]
"""
...
DECK = Deck(
    ...
    slides=[
        ...
        Slide(data=HeroData(
            eyebrow="ISDIN COMPETITIVE · 2026", title="双抗防晒攻防战",
        ...
        # 4 市场底色 KPI
        Slide(data=KpiData(
            eyebrow="CORE MARKET · 2025", title="市场底色四个数",
            stats=[
        ...
        Slide(data=TableData(
            eyebrow="HEAD TO HEAD", title="五品牌投放对照",
        ...
        Slide(data=IconGridData(
            eyebrow="P09 · CONTENT PLAYS", title="内容需求线与杠杆",
        ...
        Slide(data=TimelineData(
            eyebrow="RHYTHM", title="全年声量节奏", subtitle="DUAL PEAK + SUMMER PLATEAU",
            milestones=[
        ...
        # 12 概念占位表
        Slide(data=TableData(
            eyebrow="MINDSHARE MAP", title="科技概念占位战",
        ...
        Slide(data=TwoColData(
            eyebrow="PLAYBOOK 2026", title="攻防清单",
        ...
        Slide(data=QuoteData(
            quote="双抗写在配方里,也要写进心智里。", attribution="—— 2026 攻防主线")),
        # 16 收尾
        Slide(data=ClosingData(
            title="把差异化讲出来。", subtitle="Own the anti-heat-aging story",
...
if __name__ == "__main__":
    dest = ROOT / "out" / "isdin_riso"
    out = sys.argv[1] if len(sys.argv) > 1 else str(dest / "isdin_riso.pptx")
