...
- **叙事段 lead**：双线规下 1~2 句承接正文（内容页必填，`density.lead_required`）——
  标题给判断、lead 给语境、证据主体给数字、SO WHAT 给推论，一页四段式。
- **chart 证据页三件套**：`subtitle` 单位·口径·周期行 + `note` 图上注释框
  （虚线深红）+ `source` 来源页脚——单图表页也要有完整证据链。
...
`**强调**`）→ 左主图表模块通高（栏头 + 原生图表 + 虚线标注框）→ 右栏
垂直堆叠：侧栏结构条模块（横条按组内最大值归一，`em` 条上深红）+ ①②
编号洞察框（红点分条，点内 `**数字**` 渲染深红加粗）→ 底部「对企业的
意义」条（深红标签 + 图标四栏）→ 来源页脚。左图右释：图上现象由右侧
编号洞察解读。IR 侧是新增的
