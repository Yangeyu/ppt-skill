# ppt-tool —— 素材进,咨询级原生 PPT 出

**本仓库是一个 agent skill**([`SKILL.md`](SKILL.md) 在根):把整个仓库放进(或软链到)
...
| 形态 | 入口 | 说明 |
|---|---|---|
| **skill** | `SKILL.md` | agent 装载仓库,按快路径四步走(默认形态) |
...
| **mastra** | `cd mastra && pnpm generate <素材.md> <look>` | 外部真 agent e2e(qwen3.7-plus),验证整条链路 |
...
mastra/        外部 agent 消费者(deckGenerator e2e + visionCritic 识图评审)
